...
/************************************************************/
 ...
 void execute_ADD(){
   int nextValue;
   if(INST.A == 0)	 
      nextValue = CURRENT_LATCHES.REGS[INST.SR1] + CURRENT_LATCHES.REGS[INST.SR2];
   else
      nextValue = CURRENT_LATCHES.REGS[INST.SR1] + SEXT(INST.imm5, 5);
   NEXT_LATCHES.REGS[INST.DR] = Low16bits(nextValue);
   setcc(nextValue);
 }
 ...
 void execute_BR(){
   if((INST.n && CURRENT_LATCHES.N) || (INST.z && CURRENT_LATCHES.Z) || (INST.p && CURRENT_LATCHES.P))
     NEXT_LATCHES.PC = Low16bits(NEXT_LATCHES.PC + LSHF( SEXT(INST.PCoffset9, 9), 1));
 }

 void execute_LDB(){
   int nextValue = SEXT(read_byte(Low16bits(CURRENT_LATCHES.REGS[INST.BaseR] + SEXT(INST.boffset6, 6))), 8);
   NEXT_LATCHES.REGS[INST.DR] = Low16bits(nextValue);
   setcc(nextValue);
 }
 ...
 void execute_LDW(){
   int nextValue = read_word( Low16bits(CURRENT_LATCHES.REGS[INST.BaseR] + LSHF( SEXT( INST.boffset6, 6), 1)));
   NEXT_LATCHES.REGS[INST.DR] = Low16bits(nextValue);
   setcc(nextValue);
 ...
 void execute_STW(){
   write_word( Low16bits(CURRENT_LATCHES.REGS[INST.BaseR] + LSHF(SEXT( INST.boffset6, 6), 1)), CURRENT_LATCHES.REGS[INST.DR]);
 }
 ...
 void execute_SHF(){
   int nextValue;
   if(INST.D == 0){
      nextValue = LSHF(CURRENT_LATCHES.REGS[INST.SR1], INST.amount4);
   }
   else{
      if(INST.A == 0){
         nextValue = RSHF(CURRENT_LATCHES.REGS[INST.SR1], INST.amount4, 0);
      }
      else{  
        int topbit = get_bits( CURRENT_LATCHES.REGS[INST.SR1], 15, 15);
        nextValue = RSHF(CURRENT_LATCHES.REGS[INST.SR1], INST.amount4, topbit);
      }
   }
   NEXT_LATCHES.REGS[INST.DR] = Low16bits(nextValue);
   setcc(nextValue);
